...
上面存的是K值，还需要看下W值，因为下半部分需要减去$W_N^K$，用第二前四个行来举例,b就是代码里面的butterflyIndex，m等于4，所以到时候垂直和水平IFFT的时候只需要加下去就行了，不用变更符号。
$$
e^{i⋅2π⋅(b+\frac{m}{2})/m}=−e^{i⋅2π⋅b/m}
$$

|   |y0| y1 |y2 |y3   |
| ---  | --- |--- |--- |---  |
| K | 0,2 |1,3 |0,2 |1,3  |
| b | 0 |1 |2 = 0 + m / 2 |3 = 1 + m / 2  |
| W | $e^{i⋅2π⋅(0)/4}$ |$e^{i⋅2π⋅(1)/4}$ |$-e^{i⋅2π⋅(0)/4}$ |$-e^{i⋅2π⋅(1)/4}$  |
```hlsl
[numthreads(1, 64, 1)]
...
这里还做了一个打包，本来是dx做一次IFFT变成实数Dx，dz做一次IFFT变成实数Dz，就是最后位移贴图的xz分量，不过这个过程做了两次IFFT，打包之后变成一次。

$$Z = dx + i \cdot dz$$

$$\text{IFFT}(Z) = \text{IFFT}(dx + i \cdot dz)$$

$$= \text{IFFT}(dx) + i \cdot \text{IFFT}(dz)$$

$$= D_x + i \cdot D_z$$

所以现在就是要计算Z，然后dx和dz本来就是复数：

$$Z = (dx_x + i \cdot dx_y) + i \cdot (dz_x + i \cdot dz_y)$$

$$= dx_x + i \cdot dx_y + i \cdot dz_x - \underbrace{dz_y}_{i^2 = -1}$$

$$= \underbrace{(dx_x - dz_y)}_{Z_x} + i \underbrace{(dx_y + dz_x)}_{Z_y}$$
```hlsl
...
# IFFT垂直和水平
这里_Twiddle保存的K值和$w = e^{i2\pi k}$用来逆傅里叶变换，因为一张贴图不能同时写入和读取，所以用了两张贴图来回存储和读取，复指数的相乘用的都是欧拉公式代替的，因为CSPrecomputeTwiddle里面已经做好准备了，这边计算完直接相加就好了。
...
# 重心转换和位移图
因为频谱计算完重心是在左上角的，所以需要将重心移到中心去。在频域里面就是将K-N/2，下面是转换到时域的变换过程：

$$
g[x] = \frac{1}{N} \sum_{k=0}^{N-1} F\left[k - \frac{N}{2}\right] \cdot e^{i2\pi\frac{kx}{N}}
$$

$$
g[x] = \frac{1}{N} \sum_{k=0}^{N-1} F\left[k'\right] \cdot e^{i2\pi\frac{(k - N/2)x}{N}}
$$

$$
g[x]= \frac{1}{N} \sum_{k'} F[k'] \cdot e^{i2\pi\frac{k'x}{N}} \cdot \underbrace{e^{i2\pi\frac{N/2 \cdot x}{N}}}_{e^{i\pi x} = (-1)^x}
$$
...
得出就是额外乘上$(-1)^x$，就是基数为-1偶数为1。_Lambda控制波浪的强度。然后在存储值的时候存储的是$(h_r, h_i, D_x, D_z)$，$h_r,h_i$这两个做完变换得到实数高度值存在了x分量，所以最后就是xzw组成最后的位移图。
...
void CSAssemble(uint3 id : SV_DispatchThreadID)
{
    ...
    float4 v = readSpectrum(id.xy, _PingPong);
    float sgn = (((id.x + id.y) & 1u) == 0u) ? 1.0 : -1.0;

    float h  = v.x * sgn;
    float Dx = v.z * sgn;
    float Dz = v.w * sgn;

    _Displacement[id.xy] = float4(Dx * _Lambda, h, Dz * _Lambda, 1.0);
}
```

# 法线和浪沫
法线的求解是用的三维空间中的曲面参数方程，h是高度，$D_x, D_z$两个参数用来偏移位置的，$\lambda$是缩放因子，$h,D_x, D_z$都是在u,v处的值：
$$P(u, v) = (u + \lambda D_x(u, v), \quad h(u, v), \quad v + \lambda D_z(u, v))$$

这个的偏导就是切线向量，先对$u$进行偏导：
X分量偏导：
$$\frac{\partial}{\partial u}(u + \lambda D_x) = 1 + \lambda \frac{\partial D_x}{\partial u} = 1 + \frac{\partial(\lambda D_x)}{\partial u}$$

Y分量偏导：
$$\frac{\partial h}{\partial u}$$

Z分量偏导：
$$\frac{\partial}{\partial u}(v + \lambda D_z) = 0 + \frac{\partial(\lambda D_z)}{\partial u} = \frac{\partial(\lambda D_z)}{\partial u}$$

所以得出：
$$T_u = \left( 1 + \frac{\partial \lambda D_x}{\partial u}, \quad \frac{\partial h}{\partial u}, \quad \frac{\partial \lambda D_z}{\partial u} \right)$$
同理得出对$v$偏导：
$$T_v = \left( \frac{\partial \lambda D_x}{\partial v}, \quad \frac{\partial h}{\partial v}, \quad 1 + \frac{\partial \lambda D_z}{\partial v} \right)$$

然后$\partial u,\partial v$就是相邻位置的差值，得出两个切线之后叉乘就好了：

```hlsl
    float dxStep = _L / float(_N);

    uint2 pR = uint2((id.x + 1u) % _N, id.y);
    uint2 pL = uint2((id.x + _N - 1u) % _N, id.y);
    uint2 pU = uint2(id.x, (id.y + 1u) % _N);
    uint2 pD = uint2(id.x, (id.y + _N - 1u) % _N);

    float4 cR = _Displacement[pR];
    float4 cL = _Displacement[pL];
    float4 cU = _Displacement[pU];
    float4 cD = _Displacement[pD];

    float dhdu  = (cR.y - cL.y) / (2.0 * dxStep);
    float dhdv  = (cU.y - cD.y) / (2.0 * dxStep);
    float dDxdu = (cR.x - cL.x) / (2.0 * dxStep);
    float dDxdv = (cU.x - cD.x) / (2.0 * dxStep);
    float dDzdu = (cR.z - cL.z) / (2.0 * dxStep);
